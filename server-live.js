...
app.get('/', (req,res)=> res.send('Sahchar Live v6'));
...
wss.on('connection', (ws)=>{
  ...
  async function processAudio(){
    ...
    try{
      ...
      const comp=await openai.chat.completions.create({model:'gpt-4o-mini',messages:history,max_tokens:120,temperature:0.7});
...
console.log(`🔊 Sent ${pcm.length} bytes`);
  ...
  ws.on('message',(data,isBinary)=>{
    if(!isBinary){ try{ const j=JSON.parse(data.toString()); if(j.type==='barge-in'){ stopTTS=true; isBotSpeaking=false; console.log('🔴 Barge-in'); safeSend(JSON.stringify({type:'barge-in-ack'})); } }catch{}; return; }
