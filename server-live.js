...
app.get('/', (req,res)=> res.send('Sahchar Live Dual v8'));
...
wss.on('connection', (ws, req)=>{
  ...
  const isFullDuplex = url.searchParams.get('full') === '1';
  
  console.log(`🔌 Connected ${isFullDuplex ? 'FULL' : 'HALF'}-duplex`);
  ...
  ws.on('message',(data,isBinary)=>{
    if(!isBinary){ 
      try{ 
        ...
        // सिर्फ full-duplex में barge-in allow करो
        if(isFullDuplex && j.type==='barge-in'){ 
