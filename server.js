...
// ==================== 5. IMAGE GENERATION - OPENAI VERSION ====================
  ...
  if (openaiApiKey) {
    try {
      ...
      if (response.data[0]?.url) {
        console.log("✅ Generated by DALL-E 3");
        return res.json({ imageUrl: response.data[0].url });
      }
    } catch (e) { console.log("❌ OpenAI Failed, trying Replicate..."); }
  }

  // STEP 2: Try Replicate
  if (replicateToken) {
    try {
      ...
      if (response.ok && data.id) {
         // ... (Add your polling logic here or simplified response)
         // If polling is too complex for this block, proceed to final fallback
      }
    } catch (e) { console.log("❌ Replicate Failed, trying Pollinations (FREE)..."); }
  }

  // STEP 3: FINAL FREE FALLBACK (Pollinations.ai) - Always Works!
  console.log("🚀 Using Pollinations.ai (Free/Stable)");
  const freeUrl = `https://pollinations.ai/p/${encodeURIComponent(prompt)}?width=1024&height=1024&seed=${Math.floor(Math.random()*1000)}&nologo=true`;
  return res.json({ imageUrl: freeUrl, provider: "pollinations" });
});
// ==================== 6. IMAGE ANALYZE ====================
