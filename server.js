...
// ==================== 5. IMAGE GENERATION - MULTI-PROVIDER FALLBACK ====================
app.post("/api/image/generate", async (req, res) => {
  ...
  if (openaiKey && openaiKey.startsWith('sk-')) {
    try {
      console.log(`🖼️ Trying OpenAI DALL-E 3 for: ${cleanPrompt.substring(0, 40)}...`);
      ...
      console.log(`⚠️ OpenAI DALL-E 3 failed: ${e.message}`);
      // Try DALL-E 2 as fallback within OpenAI
      try {
        ...
        if (response.data && response.data[0] && response.data[0].url) {
          console.log(`✅ Image generated by DALL-E 2`);
          return res.json({ imageUrl: response.data[0].url, provider: "dall-e-2" });
        }
      } catch (e2) {
        console.log(`⚠️ DALL-E 2 also failed: ${e2.message}`);
      }
    }
  }
  ...
  if (replicateToken) {
    try {
      console.log(`🖼️ Trying Replicate SDXL for: ${cleanPrompt.substring(0, 40)}...`);
      ...
      if (response.ok) {
        ...
        if (imageUrl) {
          console.log(`✅ Image generated by Replicate SDXL`);
  ...
  // ========== PROVIDER 3: POLLINATIONS.AI (FINAL FALLBACK - ALWAYS WORKS) ==========
  console.log(`🚀 Using Pollinations.ai (Final Fallback - Free & Reliable)`);
  ...
  console.log(`✅ Image URL generated via Pollinations`);
  res.json({ 
    imageUrl: imageUrl, 
    provider: "pollinations",
    note: "Free fallback provider - Always works"
  });
});
// ==================== 6. IMAGE ANALYZE ====================
